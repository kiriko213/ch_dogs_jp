...
def normalize_text_for_speech(text, language="ja"):
    ...
    - アルファベットや誤読されやすい単語の読みをひらがな・カタカナに変換（誤読・ロボット感の防止）
    """
    if language == "ja":
        # 1. アルファベットの誤読防止
        ...
        # 2. 動物チャンネルに頻出する漢字の誤読・不自然なイントネーションの防止（ひらがな化/カタカナ化）
        pronunciation_map = {
            "愛犬": "あいけん",
            "愛猫": "あいびょう",
            "飼い主": "かいぬし",
        ...
        # 3. 句読点を挿入して Edge TTS の「間（ま）」を自然にする
        text = text.replace("じつは", "じつは、").replace("なんと", "なんと、")
        text = text.replace("しかし", "しかし、").replace("だから", "だから、")
        
        # 4. 文末に句点がない場合に補完（最後の間を空けるため）
